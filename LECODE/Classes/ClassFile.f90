...
!> \file ClassFile.f90
!
! Description :  It encapsulates the file module used by SPModel during compilation.
!
!<
! ============================================================================
!> Module file_data gives the file and directory names used during SPModel simulations.
!<
module file_data
   ...
   character(len=128) :: faccu, faccucpp
   ! Fill depression dem file
   character(len=128) :: fdem, fdemcpp
   ! TIN output files
   character(len=128) :: ftin
   ! TIN nodes file
   character(len=128) :: fnodes
   ! TIN nodes & ghosts file
   character(len=128) :: fnode
   ! Strata output files
   ...
   ! Temperature and salinity fluctuation file
   character(len=128) :: ftempsal
   ! Rain level file
   character(len=128) :: frain
   ...
   ! Seismic line ascii file
   character(len=128) :: fseismic
   ! RMS block ascii file
   character(len=128) :: frms
   ! Underworld maestro file
   character(len=128) :: maestro
   ! Ocean maestro file
   character(len=128) :: poseidon
   ! Ocean top surface file
   character(len=128) :: foceanxyz
   ! Ocean top surface file
   character(len=128) :: foceanout
   ! Deposit files
...
20    continue
   ...
   subroutine get_pointed_file_name( line )
        ...
        integer :: iut, ios, position, position2, len
        logical :: find, found
